...
Initial security surface — interface-only primitives + auth middlewares that plug into `@quilla-kit/http`'s Router.
...
Consumers compose the typed `AuthMiddlewareStack` (from `@quilla-kit/http`) directly: `{ tokenVerification: bearerTokenMiddleware({ tokenService }), sessionLoad: authenticatedSessionMiddleware({ sessionStore, executionContextProvider }) }`. No builder helper — the phase shape is the API.
...
Serves as the toolkit's rule-of-three validation harness: the first consumer that composes every other quilla-kit package together.
